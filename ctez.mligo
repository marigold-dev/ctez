...
let create (s : storage) (create : create) : result =
  ...
  if Big_map.mem handle s.ovens then
    ...
    let oven = {tez_balance = (Tezos.get_amount ()) ; ctez_outstanding = 0n ; address = oven_address}  in
...
let withdraw (s : storage) (p : withdraw)   : result =
  ...
  let new_balance = match oven.tez_balance - p.amount with
    | None -> failwith error_EXCESSIVE_TEZ_WITHDRAWAL
    | Some p -> p
  in
...
let cfmm_price (storage : storage) (price_numerator : nat) (price_denominator : nat) : result =
  if (Tezos.get_sender ()) <> storage.cfmm_address then
    ...
    let target_less_price : int = new_target - price in
    let d_drift =
    ...
    let txndata_ctez_target = new_target in
